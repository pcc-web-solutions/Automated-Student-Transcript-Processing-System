<?php
	...
	$course_codes=$conn->query("SELECT DISTINCT(results_entry.course_code) AS codes FROM results_entry INNER JOIN courses ON courses.code = results_entry.course_code INNER JOIN trainees ON trainees.course_code = results_entry.course_code WHERE courses.department_code = '$dept' AND term = '$term' AND exam_year = '$year' AND trainees.status = '1' ORDER BY results_entry.course_code ASC");
	...
	//PDF Report title
	$pdf->Ln();
	$pdf->WriteHTMLCell(193,5,'','','<h2 style="text-align: center;" ><u>'.strtoupper($term).' '.$year.' UNSUBMITTED MARKS</u></h2>',0);
	$pdf->Ln(10);
