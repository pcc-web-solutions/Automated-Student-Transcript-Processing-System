...
if(isset($_POST['selected_course']) && isset($_POST['selected_unit']) && isset($_POST['selected_class']))
{
	...
	elseif($class == "" ){
		...
		if(mysqli_num_rows($trainees)<1){
			...
			<button type="button" name='.$status.' class="btn btn-info float-right" id=save style=width:50%>SAVE</button>
...
<style>	
  ...
  $('#save').click(function(){
  	...
  	else{toastr.info("Mark entry session is currently closed")}
  })
	})
  </script>
