...
function getTraineesPerDepartment(){
    global $conn; 
    $depts = array();
    $sql = $conn->query("SELECT DISTINCT departments.department_code, departments.abbreviation, departments.department_name, COUNT(DISTINCT trainees.adm) AS trainees FROM departments INNER JOIN courses ON courses.department_code = departments.department_code INNER JOIN trainees ON trainees.course_code = courses.code GROUP BY departments.department_code ORDER BY departments.department_name ASC;");
    if($sql->num_rows>0){
        $depts = array();
        while($row=$sql->fetch_array()){
            ...
            $depts[] = array("deptcode"=>$deptcode, "abbreviation"=>$abbreviation, "deptname"=>$deptname,"trainees"=>$trainees);
        }
        $response = array("status"=>"success","data"=>$depts);
    }else{
        $response = array("status"=>"error","message"=>"No departments found");  
    }
...
if(isset($_POST['req'])) {
    if ($_REQUEST['req'] == 'department_data') {
        echo getTraineesPerDepartment();
    }
