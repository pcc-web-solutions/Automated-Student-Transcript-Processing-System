<?php
	...
	while($array_data=$get_unit_data->fetch_array())
	{
		...
		$select_trainees = $conn->query("SELECT DISTINCT(trainees.adm), trainees.name FROM trainees INNER JOIN courses ON courses.code = trainees.course_code INNER JOIN classes ON classes.class_name = trainees.class WHERE courses.code = '$course' AND trainees.class = '$class' AND trainees.status = '1' ");
	...
	$_SESSION['ed'] = NULL;
	$pdf->Output($term.' '.$year.' EXAM ATTENDANCE SHEET.pdf');
?>