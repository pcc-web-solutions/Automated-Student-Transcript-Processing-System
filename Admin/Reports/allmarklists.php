<?php
	...
	$unit_codes = $conn->query("SELECT DISTINCT(units.unit_code), units.unit_name,courses.code, courses.course_name, if(CHAR_LENGTH(courses.code) >=5, substring_index(courses.code, '/', -1), 'N/A') AS module, departments.department_code, departments.department_name FROM results_entry INNER JOIN units ON units.unit_code = results_entry.unit_code INNER JOIN courses ON courses.code = units.courses_code INNER JOIN trainees ON trainees.course_code = courses.code INNER JOIN departments ON departments.department_code = courses.department_code WHERE results_entry.term = '$term' AND results_entry.exam_year = '$year' AND trainees.status = '1' ORDER BY courses.code, units.unit_code ASC");
	...
	foreach($units as $array_data)
	{
		...
		while($classes_data=mysqli_fetch_array($select_classes)){
			...
			if($number_of_trainees > 0){
				...
				$pdf->SetFont('Times', 'B', 10);
				$pdf->Cell(35,5,'ADM. NO',1,0,'L');
				$pdf->Cell(55,5,'FULL NAME',1,0,'L');
				...
				while($trainee = mysqli_fetch_assoc($select_trainees)){
					...
					while($marks=mysqli_fetch_assoc($trainee_marks)){
						...
						$pdf->SetFont('Times', '', 10);
						$pdf->Cell(35,5,$trainee_adm,1,0,'L');
						$pdf->Cell(55,5,$trainee_name,1,0,'L');
