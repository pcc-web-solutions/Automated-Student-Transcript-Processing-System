<?php
	...
	$sql="SELECT distinct(adm) as adm from results_entry where adm='$adm_number'";
	$record_count=mysqli_query($conn, $sql);
	while($rows=mysqli_fetch_assoc($record_count))
	{
		$admissions[]=$rows;
	}
	...
	on courses.department_code =departments.department_code) where trainees.adm='$adm_number' LIMIT 1");
	...
	foreach($admissions as $admission)
	{
		...
		//Watermark
		$pdf->Image('../../Images/transcript-watermark.jpg',80,90,60);
				...
				<table>
					<tbody style="font: 10px";>
						...
						<tr>
							...
							<td style = "width: 220px;">'.strtoupper($row['course_name']).'</td>
							<td><b>TERM:</b></td>
		...
		{ 
			...
			$unitcode = strtoupper($row['unit_code']);
			$cat = $row['cat'];
			$exam = $row['exam'];
			...
			// $pdf->Cell(20,5,$sn,1,0);
			$pdf->Cell(20,5,$unitcode,1,0);
	...
	$pdf->Output($trname.' '.$year.' '.strtoupper($term).' TRANSCRIPT.pdf', true);
?>
