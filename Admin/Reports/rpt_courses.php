...
if(!isset($_SESSION['Admin'])){
	...
	$get_courses = $conn->query("SELECT DISTINCT(courses.code), courses.course_name, courses.course_abrev, courses.department_code, departments.department_code, departments.department_name FROM courses INNER JOIN units ON units.courses_code = courses.code INNER JOIN departments ON departments.department_code = courses.department_code ORDER BY courses.code, courses.course_name ASC");
	if($get_courses->num_rows>0){
		...
		while ($rows = mysqli_fetch_array($get_courses)) {
			...
			$department_name = strtoupper($rows['department_name']);
		...
		$pdf->Output('UNITS PER COURSE.pdf');
	}
}
?>