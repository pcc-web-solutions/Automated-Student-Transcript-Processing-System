		...
		if(isset($_POST['selected_trainer'])){
			if(isset($_POST['selected_course'])){
				if(isset($_POST['selected_class'])){
					...
					$unit = array();
					$unitsquery=$conn->query("SELECT unit_code, unit_name FROM units WHERE courses_code='$selected_course' AND unit_code NOT IN (SELECT trainer_units.unit_code FROM trainer_units INNER JOIN units ON units.unit_code = trainer_units.unit_code WHERE courses_code='$selected_course' AND trainer_units.class_name = '$selected_class' AND trainer_units.trainer_id = '$selected_trainer' ORDER BY trainer_units.unit_code) ORDER BY unit_code");
							...
							<tbody>
								...
								while($row = mysqli_fetch_assoc($unitsquery) ){
									...
									$unit_name = strtoupper($row['unit_name']); ?>
										<tr id="records">
											<td><center><input type="checkbox" class="unitchoice" name="unitchoice[]" id=<?php echo $unit_code; ?> ></input></center></td>
											<td><?php echo ++$sn; ?></td>
