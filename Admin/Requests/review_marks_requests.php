<?php
	...
	if(isset($_POST['request'])){
		...
		elseif ($request == "Delete all marks") {
			...
			// echo "Deleting this session marks";

			$delete_marks = $conn->query("DELETE FROM results_entry WHERE exam_year = '$year' AND term = '$term'");
			if(!$delete_marks){
				...
				$confirm_deletion = $conn->query("SELECT COUNT(sn) AS records FROM results_entry WHERE exam_year = '$year' AND term = '$term'" );
                while($rows = mysqli_fetch_assoc($confirm_deletion)){
                    $deleted = $rows['records'];
                }
				if ($deleted==0) {
					echo "<div class='alert alert-success'><strong>Success!</strong> Term $term $year marks deleted successfully.</div>";
				}
				else{
