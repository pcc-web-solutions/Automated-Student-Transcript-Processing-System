<?php
	...
	if ($course_codes->num_rows<1) {

		// code...

		echo "You haven't entered marks for this session yet.";
	...
	$pdf->WriteHTMLCell(193,5,'','','<h2 style="text-align: center;" ><u>'.strtoupper($term).' '.$year.' MISSING MARKS </u></h2>',0);
...

	{
		...
		$courses=$conn->query("SELECT courses.code, courses.course_name, COUNT(units.unit_code) AS total_units_for_course FROM courses INNER JOIN units ON units.courses_code = courses.code WHERE code = '$code' ORDER BY courses.code ASC LIMIT 1");
			...
			while($row=mysqli_fetch_assoc($classes)){
				...
				$units=$conn->query("SELECT DISTINCT(trainer_units.unit_code), units.unit_name, trainers.first_name, trainers.last_name FROM trainer_units INNER JOIN units ON trainer_units.unit_code = units.unit_code INNER JOIN trainers ON trainers.trainer_id = trainer_units.trainer_id INNER JOIN classes ON classes.class_name = trainer_units.class_name WHERE trainer_units.trainer_id = '$trainer_id' AND units.courses_code = '$code' AND trainer_units.class_name = '$class' AND trainer_units.unit_code NOT IN (SELECT DISTINCT(results_entry.unit_code) FROM results_entry INNER JOIN trainees ON trainees.adm = results_entry.adm INNER JOIN classes ON classes.class_name = trainees.class WHERE results_entry.course_code = '$code' AND classes.class_name = '$class' AND term = '$term' AND exam_year = '$year') GROUP BY units.unit_code ORDER BY units.unit_code ASC");
				...
				if($units->num_rows>0){
...

					{
						...
						$pdf->Cell(40,5,strtoupper($row['first_name']." ".$row['last_name']),1,0);
