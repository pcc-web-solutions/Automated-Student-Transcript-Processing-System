<?php
	...
	$unit_codes = $conn->query("SELECT DISTINCT(trainer_units.unit_code), units.unit_name, courses.code, courses.course_name, IF(CHAR_LENGTH(courses.code) >= 5 , SUBSTRING_INDEX(courses.code, '/', -1),'N/A' ) AS module, departments.department_code, departments.department_name, department_hods.hod AS hod_code, trainers.first_name, trainers.last_name FROM trainer_units INNER JOIN results_entry ON trainer_units.unit_code = results_entry.unit_code INNER JOIN units ON units.unit_code = trainer_units.unit_code INNER JOIN courses ON courses.code = results_entry.course_code INNER JOIN departments ON departments.department_code = courses.department_code LEFT JOIN department_hods ON department_hods.department_code = departments.department_code LEFT JOIN trainers ON trainers.trainer_id = department_hods.hod WHERE trainer_units.trainer_id = '$trainer_id' AND results_entry.term = '$term' AND results_entry.exam_year = '$year' ORDER BY unit_code ASC");
...

	{
		...
		$hodname = strtoupper($array_data['first_name'].' '.$array_data['last_name']);
		...
		$select_classes=$conn->query("SELECT DISTINCT(class_name) FROM trainer_units INNER JOIN results_entry ON results_entry.unit_code = trainer_units.unit_code INNER JOIN trainees ON trainees.class = trainer_units.class_name WHERE trainer_id = '$trainer_id' AND trainer_units.course_code = '$coursecode' ORDER BY trainer_units.class_name DESC");
		...
		while($classes_data=mysqli_fetch_array($select_classes)){
			...
			$select_trainer = $conn->query("SELECT trainers.first_name, trainers.last_name FROM trainer_units INNER JOIN trainers ON trainers.trainer_id = trainer_units.trainer_id WHERE trainer_units.class_name = '$classname' ");
			...
			$pdf->Cell(30,5,'H.O.D NAME:        ________________________________',0,0,'L');

			$pdf->SetFont('Times', '', 10);

			$pdf->Cell(60,5,' '.$hodname,0,0,'L');
		...
		$pdf->Cell(190,5,'TRAINER NAME:',0,0);
