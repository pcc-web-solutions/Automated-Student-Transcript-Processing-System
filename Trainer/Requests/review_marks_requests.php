<?php
	...
	if(isset($_POST['request'])){
		...
		if($request == "View all marks"){
			...
			$select_all_marks = $conn->query("SELECT DISTINCT(results_entry.sn), results_entry.adm, trainees.name, results_entry.course_code, classes.class_name, results_entry.unit_code, units.unit_name, cat, exam, exam_year, term FROM results_entry INNER JOIN units ON units.unit_code = results_entry.unit_code INNER JOIN trainees ON trainees.adm = results_entry.adm INNER JOIN classes ON classes.class_name = trainees.class INNER JOIN courses ON courses.code = results_entry.course_code INNER JOIN trainer_units ON trainer_units.unit_code = results_entry.unit_code WHERE trainer_units.trainer_id = '$trainer_id' AND term = '$term' AND exam_year = '$year' ORDER BY results_entry.course_code, classes.class_name, results_entry.adm DESC");
			...
			<?php
				...
				echo '<div class="alert alert-warning"><strong>Sorry!</strong> You have not entered marks for this term.</div>';
		...
		elseif ($request == "Delete all marks") {
			...
			if($select_adm_no->num_rows>0){
				...
				echo "<div class='alert alert-success'><strong><i class = 'fa fa-check text-white'></i>&nbspSuccess!</strong><br>Marks deleted successfully.</div>";
            ...
            $select_serial_numbers = $conn->query("SELECT DISTINCT(results_entry.sn) FROM results_entry INNER JOIN units ON units.unit_code = results_entry.unit_code INNER JOIN trainees ON trainees.adm = results_entry.adm INNER JOIN classes ON classes.class_name = trainees.class INNER JOIN courses ON courses.code = results_entry.course_code INNER JOIN trainer_units ON trainer_units.unit_code = results_entry.unit_code WHERE trainer_units.trainer_id = '$trainer_id' AND term = '$term' AND exam_year = '$year' ORDER BY results_entry.course_code, classes.class_name, results_entry.adm DESC");

            if($select_serial_numbers->num_rows>0){

				while($rows=mysqli_fetch_array($select_serial_numbers)){
				...
				foreach ($sns as $record_no) {

					$sn = $record_no['sn'];

					$delete_marks = $conn->query("DELETE FROM results_entry WHERE sn = '$sn'");
				...
				if ($records_deleted > 0) {
					...
					echo "<div class='alert alert-success'><strong><i class = 'fa fa-check text-white'></i>&nbspSuccess!</strong><br>All your marks for ".strtoupper($term)." $year have been deleted successfully.</div>";

				}

				else{

					echo '<div class="alert alert-warning"><strong>Sorry!</strong> The system was unable to delete all the mark entries for this session. Kindly try again.</div>';
