<?php
	...
	$get_courses = $conn->query("SELECT DISTINCT(courses.code), courses.course_name FROM courses INNER JOIN trainer_units ON trainer_units.course_code = courses.code INNER JOIN trainees ON trainees.course_code = courses.code WHERE trainer_units.trainer_id = '$trainer_id' ORDER BY course_name ASC");
...
<body>
	...
	<div class="card card-info">
		...
		<div class="card-body">
			...
			<div id="sheet_specification">

				<div class="row p-0">

					<div class="col-sm-6" style="margin-bottom: 10px;"><input type="radio" class="custom-radio " name="specification" value="blank" checked>&nbsp<strong>Blank sheet</strong></div>

					<div class="col-sm-6" style="margin-bottom: 10px;"><input type="radio" class="custom-radio " name="specification" value="autofilled">&nbsp<strong>Auto-filled with trainees</strong></div>
		...
		$('#generate').click(function(){
			...
			if(selected_course == ''){
				...
				$.ajax({
					...
					data: {choice:choice, course:selected_course, class:selected_class, unit:selected_unit, exam_date:exam_date, no_of_trainees:no_of_trainees},

					success: function(data){

						toastr.success('Success. Exam attendance sheet loading successfully')

						$('.report').html('<hr style="margin-bottom: 1px"><iframe id=iframe style="border: 1px solid grey; width: 100%; height: 40vh;" ></iframe>');
